...
class MilitaryEligibilityEngine:
    ...
    def add_fact(self, key, value):
        ...
        if existing == value:
            return False
        if key in ("eyeScore", "astigmatismEyeScore") and isinstance(existing, int) and isinstance(value, int):
            if value > existing:
                self.facts[key] = value
                return True
            return False
        ...
        for key, val in cond.items():
            ...
            if isinstance(fact_val, list):
                if isinstance(val, list):
                    if not any(item in val for item in fact_val):
                        return False
                else:
                    if val not in fact_val:
                        return False
                continue
            ...
            if isinstance(val, dict):
                ...
                if "lte" in val and not (fact_val <= val["lte"]): 
                    return False
                if "equal" in val and fact_val != val["equal"]:
                    return False
                ...
                if "between" in val:
                    ...
                    if not (l <= fact_val <= h): 
                        return False
                if "or" in val:  # Hỗ trợ toán tử OR
    ...
    # --------- Forward Chaining Stack ---------
    def run(self):
        stack = list(self.facts.keys())

        while stack:
            fact_key = stack.pop()
            for rule in self.rules:
                rid = rule["ruleId"]
                if rid in [r["ruleId"] for r in self.triggered_rules]:
                    continue
                if rule.get("condition") and fact_key not in rule["condition"]:
                    continue
                if self._check_condition(rule.get("condition")):
                    self.triggered_rules.append(rule)
                    before = set(self.facts.keys())
                    self._apply_result(rule)
                    after = set(self.facts.keys())
                    new_generated = after - before
                    for nf in new_generated:
                        stack.append(nf)
...
with col2:
    ...
    hyperopia = st.checkbox("Viễn thị")
...
if st.button("KIỂM TRA NGHĨA VỤ QUÂN SỰ", type="primary", use_container_width=True):
    ...
    if myopia > 0:
        ...
        safe_add(engine, "myopiaEyeScore", myopia)
        if has_correction:
    ...
    if hyperopia:
        safe_add(engine, "astigmatismEye", True)
